# Spec: `design` (`@m2/design`)

> Source-of-truth specification for the `@m2/design` package. Defines the V2 design language (atmospheric, editorial, glassmorphic) — tokens (color, typography, radii, shadows, glass recipes, atmosphere mood presets, motion) and primitive React Native components (`<Glass>`, `<Atmosphere>`, `<Sparkle4>`, typography helpers). V1 tokens remain importable from the existing entrypoint until a future cutover.
...
V2 tokens and primitives MUST be reachable via the `@m2/design/v2` subpath. The root `@m2/design` entrypoint MAY re-export V2 names alongside V1, but MUST NOT shadow or rename existing V1 exports.
