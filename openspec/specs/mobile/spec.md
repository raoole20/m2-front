# Spec: `mobile` (`apps/mobile`)

> Source-of-truth specification for the Motomoto mobile app (`apps/mobile`, Expo SDK 55 / RN 0.83). Defines V2 visual contract, real `m2-back` integration via `@m2/api-client`, screen requirements (Login, Inbox, Conversation, Channels, AI Hub, Profile), the floating glass tab bar, and explicitly out-of-scope behaviors (WebSocket realtime, agent send-message, light mode, AI configuration UI).
...
`apps/mobile` MUST resolve the backend base URL from `process.env.EXPO_PUBLIC_API_URL` at app start. No hardcoded URL MUST remain anywhere in `apps/mobile/`. A missing or empty value MUST surface a clear, developer-visible error at first network call (not a silent `undefined` URL request). `.env.example` MUST document the LAN-IP override pattern for Android dev clients.
...
- AND on Android, where backdrop blur is unsupported or weak, the bar renders with a denser semi-opaque tint per the `<Glass>` Android fallback
...
Each of the five product screens MUST mount `<Atmosphere mood={...}>` at z-index 0 with the mood matching the screen (`inbox`, `conversation`, `channels`, `ai`, `profile`). The Login screen MAY mount an atmosphere with the `inbox` mood at `subtle` intensity (not pinned, but consistency is acceptable).
...
`apps/mobile/src/services/api.ts` (the legacy axios instance with hardcoded URL) MUST NOT exist. Mock service stubs under `apps/mobile/src/services/*` that are replaced by `@m2/api-client` calls MUST NOT exist. Mock data under `apps/mobile/src/mock/` MAY remain on disk for dev fallback, but MUST NOT be imported by any screen.

#### Scenario: No legacy axios instance

- GIVEN the `apps/mobile/` source tree
...
The following MUST NOT be implemented (deferred to future changes):
