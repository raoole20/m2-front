# Spec: `api-client` (`@m2/api-client`)

> Source-of-truth specification for the `@m2/api-client` package. Defines token store factories (`createBrowserTokenStore`, `createSecureTokenStore` via `@m2/api-client/expo` subpath), the `channels` resource, and the rules governing which exports live at the root vs. behind a platform-specific subpath. The Expo-specific factory and its peer dep (`expo-secure-store`) MUST stay isolated so web consumers do NOT pull Expo into their bundle.
...
`@m2/api-client` MUST export a factory `createSecureTokenStore()` that returns a value satisfying the existing `TokenStore` interface, backed by `expo-secure-store`. The factory MUST live behind a subpath import (`@m2/api-client/expo`) so that web consumers do NOT pull `expo-secure-store` into their bundle.
...
`Channel` and the create/update/connect payload types MUST be defined in or sourced from `@m2/types` (manual mirror of m2-back DTOs is acceptable). The resource MUST attach the access token from the configured `TokenStore` to every request.
...
`@m2/api-client`'s root `index.ts` MUST re-export the `channels` resource and the existing browser store. The root entry MUST NOT export `createSecureTokenStore` directly; that name is reachable only via `@m2/api-client/expo`.
